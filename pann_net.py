...
class EulerCell_DAB(nn.Module):
    ...
    def __init__(self, dt, Lr, RL, n, **kwargs):
        ...
        self.Lr = nn.Parameter(torch.Tensor([Lr])) # define trainable neural parameter Lr
        self.RL = nn.Parameter(torch.Tensor([RL])) # define trainable neural parameter RL
    ...
    def forward(self, inputs, states):
        ...
        iL_next = (self.Lr/(self.Lr+self.RL*self.dt))*states[:, 0]+\
            (self.dt/(self.Lr+self.RL*self.dt))*(inputs[:, 0]-self.n*inputs[:, 1]) # physics of iL
        return iL_next[:, None]
